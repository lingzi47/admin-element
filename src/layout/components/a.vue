    ...
    >
      <template v-for="m in routersList">
        <template v-if="!m.hidden && m.noDropdown">
          <template v-for="mf in m.children">
            <!-- 一般为控制台 -->
            <el-menu-item :index="mf.path" :key="mf.name">
        ...
        <template
          v-if="
            !m.hidden && !m.noDropdown && m.children && m.children.length > 0
          "
        >
          <!-- 一级菜单 -->
          <el-submenu :index="m.path" :key="m.name">
            <template slot="title">
              <i :class="m.meta.icon"></i><span>{{ m.meta.title }}</span>
            </template>
            <template v-for="cm1 in m.children">
              <template
                v-if="!cm1.hidden && cm1.children && cm1.children.length > 0"
              >
                <!-- 三级菜单 -->
                <el-submenu :index="m.path + '/' + cm1.path" :key="cm1.name">
                  <template slot="title">{{ cm1.meta.title }}</template>
                  <template v-for="cm2 in cm1.children">
                    <el-menu-item
                      :index="m.path + '/' + cm1.path + '/' + cm2.path"
                      :key="cm2.name"
